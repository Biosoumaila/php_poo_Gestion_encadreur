<div class="container">
    <h2>Liste des Étudiants Inscrits</h2>

    <?php if (isset($_SESSION['user_id']) && isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'): ?>

    <?php if (!empty($etudiants)): ?>
    <table class="table table-striped">
        <thead>
            <tr>
                <th>ID</th>
                <th>Nom</th>
                <th>Prénom</th>
                <th>Email</th>
                <th>Filière</th>
                <th>Année de formation</th>
                <th>Encadreur</th>
                <th>Actions</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($etudiants as $etudiant): ?>
            <tr>
                <td><?= htmlspecialchars($etudiant->getId()) ?></td>
                <td><?= htmlspecialchars($etudiant->getNom()) ?></td>
                <td><?= htmlspecialchars($etudiant->getPrenom()) ?></td>
                <td><?= htmlspecialchars($etudiant->getEmail()) ?></td>
                <td><?= htmlspecialchars($etudiant->getFiliere()) ?></td>
                <td><?= htmlspecialchars($etudiant->getAnneeFormation()) ?></td>
                <td>
                    <?php if ($etudiant->getEncadreur()): ?>
                    <?= htmlspecialchars($etudiant->getEncadreur()->getNom()) ?>
                    <?= htmlspecialchars($etudiant->getEncadreur()->getPrenom()) ?>
                    <?php else: ?>
                    <span class="badge badge-warning">Non affecté</span>
                    <?php endif; ?>
                </td>
                <td><a href="index.php?action=affecter_encadreur_admin&etudiant_id=<?= $etudiant->getId() ?>"
                        class="btn btn-sm btn-primary">Affecter</a> <a href="#" class="btn btn-sm btn-info">Voir
                        profil</a></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <?php else: ?>
    <div class="alert alert-info">
        Aucun étudiant n'est inscrit pour le moment.
    </div>
    <?php endif; ?>

    <div class="mt-3">
        <a href="index.php?action=dashboard_admin" class="btn btn-secondary">Retour au tableau de bord</a>
    </div>

    <?php else: ?>
    <div class="alert alert-danger">Vous n'êtes pas connecté en tant qu'administrateur.</div>
    <p><a href="index.php?action=connexion_admin">Se connecter</a></p>
    <?php endif; ?>
</div>